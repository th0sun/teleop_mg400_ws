...
class DobotHardware:
    ...
    def __init__(self) -> None:
        ...
        self.__cp_ratio = 0
        self.__q_target: np.ndarray = np.array([0.0] * 6)
        self.__qd_target: np.ndarray = np.array([0.0] * 6)
        self.__qdd_target: np.ndarray = np.array([0.0] * 6)
        self.__i_target: np.ndarray = np.array([0.0] * 6)
        self.__m_target: np.ndarray = np.array([0.0] * 6)
        self.__q_actual: np.ndarray = np.array([0.0] * 6)
        self.__qd_actual: np.ndarray = np.array([0.0] * 6)
        self.__i_actual: np.ndarray = np.array([0.0] * 6)
        self.__actual_i_TCP_force: np.ndarray = np.array([0.0] * 6)
        try:
        ...
        self.__cp_ratio = 0 # 0-100
        # end (adjust value)
        self.__update_speed_acc_params()

        # args for motion command
        self.__q_init = np.array([0.0] * 6)
        self.__tool_vector_init = np.array([0.0] * 6)
        self.__q_previous = self.__q_actual.copy()
        self.__tool_vector_previous = self.__tool_vector_actual.copy()
    ...
    def motion_unstack(self):
        ...
        if self.__robot_mode in [robot_mode.MODE_RUNNING, robot_mode.MODE_JOG]:
            if self.__cp_ratio == 0:
                return True, None
            
            # True CP Blending Preemption based on physical stopping distance (Real MG400 Behavior)
            q_diff = np.abs(self.__q_target - self.__q_actual)
            stop_dist = np.max((self.__qd_actual**2) / (2 * self.__acc_j + 1e-6))
            max_diff = np.max(q_diff)
            
            # Segment length prevents swallowing dense waypoints instantly
            if hasattr(self, '_DobotHardware__q_target_previous'):
                segment_dist = np.max(np.abs(self.__q_target - self.__q_target_previous))
            else:
                segment_dist = 100.0
            
            # Blend radius proportional to CP ratio. Capped so we travel at least halfway to the current target.
            blend_radius = min(stop_dist, segment_dist * 0.5) * (self.__cp_ratio / 100.0)
            
            if max_diff > blend_radius:
                return True, None
    ...
    def set_q_target(self, q_target: List[float]):
        ...
        with self.__lock:
            try:
                ...
                self.__q_target_previous = self.__q_target.copy()
    ...
    def generate_target_in_joint(self):
        ...
        with self.__lock:
            if self.__cp_ratio > 0:
                # Bypass static array generation for True CP Blending dynamic controller
                return True
                
            self.__register_init_status()
            q_trajs = []
            len_max = 0
            for i, (q_init, q_target) in enumerate(zip(self.__q_init, self.__q_target)):
    ...
    def __q_controller(self, dt: float):
        if self.__robot_mode is robot_mode.MODE_RUNNING:
            if self.__cp_ratio > 0:
                # True CP Dynamic Velocity Controller — uses REAL dt
                q_diff = self.__q_target - self.__q_actual
                q_abs = np.abs(q_diff)

                # Check target arrival
                if np.all(q_abs < 1e-4) and self.__motion_que.empty():
                    self.__q_actual = self.__q_target.copy()
                    self.__qd_actual = np.zeros(6)
                    self.__robot_mode = robot_mode.MODE_ENABLE
                    self.__time_index = 0
                    self.__log_info_msg("The robot has reached the target position.")
                    return

                for i in range(len(self.__q_actual)):
                    direction = np.sign(q_diff[i])
                    if direction == 0:
                        target_v = 0
                    else:
                        if self.__motion_que.empty():
                            # Smooth deceleration: v = sqrt(2*a*d)
                            safe_v = float(np.sqrt(2 * self.__acc_j * q_abs[i]))
                            target_v = min(self.__speed_j, safe_v)
                        else:
                            # CP blending: maintain speed through waypoints
                            target_v = self.__speed_j

                    # Velocity integration with real dt
                    desired_v_diff = (target_v * direction) - self.__qd_actual[i]
                    acc_step = np.sign(desired_v_diff) * min(abs(desired_v_diff), self.__acc_j * dt)
                    self.__qd_actual[i] += acc_step

                    # Position integration with real dt
                    pos_step = self.__qd_actual[i] * dt

                    # Prevent overshoot in a single step
                    if abs(pos_step) > q_abs[i] and np.sign(pos_step) == direction:
                        pos_step = q_diff[i]
                        self.__qd_actual[i] = pos_step / max(dt, 1e-6)

                    self.__q_actual[i] += pos_step

            else:
                # Classic Trapezoidal Generator Execution (unchanged)
    ...
    def __update_actual_status(self, dt: float):
        try:
        ...
        # Use real dt for velocity estimation
        safe_dt = max(dt, 1e-6)
        self.__qd_actual = (self.__q_actual - self.__q_previous) / safe_dt
        self.__TCP_speed_actual = (self.__tool_vector_actual - self.__tool_vector_previous) / safe_dt

        self.__q_previous = self.__q_actual.copy()
        self.__tool_vector_previous = self.__tool_vector_actual.copy()

    def update_status(self, dt: float = None):
        """update_status — pass real dt from precision timing loop."""
        if dt is None:
            dt = self.__timestep  # fallback for backward compatibility
        with self.__lock:
            self.__q_controller(dt)
            self.__update_actual_status(dt)
