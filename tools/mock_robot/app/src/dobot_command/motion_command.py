...
class MotionCommands:
    ...
    def JointMovJ(self, args):
        """JointMovJ"""
        if self.__dobot.get_robot_mode() not in [robot_mode.MODE_ENABLE, robot_mode.MODE_RUNNING]:
            self.__dobot.log_warning_msg("The robot mode is not enable or running.")
            return False
        if len(args) < 4:
            self.__dobot.log_warning_msg("The number of arguments is invalid.")
            return False

        # Support trailing options, e.g., SpeedJ=X, AccJ=X, CP=X
        for arg in args[4:]:
            if isinstance(arg, str):
                if arg.startswith("CP="):
                    try:
                        self.__dobot.set_cp_ratio(int(arg.split("=")[1]))
                    except ValueError:
                        pass
                elif arg.startswith("SpeedJ="):
                    try:
                        self.__dobot.set_speed_j_rate(int(arg.split("=")[1]))
                    except ValueError:
                        pass
                elif arg.startswith("AccJ="):
                    try:
                        self.__dobot.set_acc_j_rate(int(arg.split("=")[1]))
                    except ValueError:
                        pass
        ...
        if accepted:
            self.__dobot.set_robot_mode(robot_mode.MODE_RUNNING)
            self.__dobot.reset_time_index()
            self.__dobot.log_info_msg("The dobot accepts JointMovJ command.")
            return True
